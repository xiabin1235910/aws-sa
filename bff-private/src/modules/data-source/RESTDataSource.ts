import { Request, RequestInit, BodyInit, Response, Headers } from 'node-fetch';
import HttpProxyAgent from "proxy-agent";
...
import { HTTPCache, ValueOrPromise } from './HTTPCache';
...
export abstract class RESTDataSource<TContext = any> {
    ...
    protected didEncounterError(error: Error | any, _request: Request) {
        // console log out the error, use winston like 2.0 instead
        // throw error;
        console.log(error)
    }

    protected parseBody(response: Response): Promise<unknown | string> {
    ...
    protected async errorFromResponse(response: Response): Promise<Error> {
    ...
    private async fetch<TResult>(
        ...
        if (process.env.NODE_ENV === 'vm') {
            if (process.env.PROXY_BAPI_ENABLE) {
                init.agent = new HttpProxyAgent(`http://${process.env.PROXY_HOST || '127.0.0.1'}:${process.env.PROXY_PORT || 8899}`)
            }
        }
        ...
        if (
            ...
            const optionHeaders = options.headers as Headers;
            if (!optionHeaders.get('Content-Type')) {
    ...
    protected async trace<TResult>(
        ...
        if (process && process.env && process.env.NODE_ENV === 'vm') {
